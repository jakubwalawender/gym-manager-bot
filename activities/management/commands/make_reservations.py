...
from activities.management.get_token import get_token
...
class Command(BaseCommand):
    ...
    def make_reservations(self, users, weekday, activity_hour):
        for instance in users:
            ...
            activities = user.activities.filter(weekday=weekday, hour=activity_hour).order_by("-hour")
            logger.info(f"{user.username} Found {len(activities)} desired activities!")
            for activity in activities:
                activity_date = (datetime.now() + timedelta(days=7)).date().strftime("%Y-%m-%d")
                activity_datetime = f"{activity_date}T{activity.hour}Z"
                data = {
                    "UserId": user.gym_manager_id,
                    "Date": activity_datetime,
                    "ClassScheduleId": activity.activity_id
                }
                ...
                if r.status_code == 200:
                    if r.json()["Status"] == ReservationStatus.RESERVE.value:
                        ...
                        for another_option in activity.another_activity_options.all():
                            requests.post(REMOVE_RESERVATION_URL, json=data, headers=headers)
                            activity_datetime = f"{activity_date}T{another_option.hour}Z"
                            data = {
                                "UserId": user.gym_manager_id,
                                "Date": activity_datetime,
